...
cmake -G "Visual Studio %VS_NUMBER% %VS_VERSION%%CMAKE_ARCH%" %CMAKE_PARAM_A% -DCMAKE_CXX_STANDARD=17 -DBUILD_APPS=OFF -DDCMTK_WIDE_CHAR_FILE_IO_FUNCTIONS=ON -DDCMTK_USE_DCMDICTPATH=OFF -DDCMTK_ENABLE_PRIVATE_TAGS=ON -DDCMTK_COMPILE_WIN32_MULTITHREADED_DLL=ON -DDCMTK_OVERWRITE_WIN32_COMPILER_FLAGS=OFF -DDCMTK_WITH_ZLIB=ON -DWITH_ZLIBINC=%zlib% -DCMAKE_INSTALL_PREFIX=%bin% -S %library% -B %build%
